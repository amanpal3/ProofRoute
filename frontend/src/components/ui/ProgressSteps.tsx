...
export default function ProgressSteps({ steps, currentIndex }: ProgressStepsProps) {
  return (
    <div className="space-y-2">
      ...
      <div className="flex justify-between text-[10px] font-mono uppercase tracking-wide text-slate-500">
        {steps.map((step, index) => (
          <span key={step} className={cn(index <= currentIndex && 'text-cyan-400')}>
            {step.replace('_', ' ')}
          </span>
        ))}
      </div>
